...
<Callout type="info">
**Type:** Self-initiated personal project — a full-featured collaborative whiteboard. **Live at** [CoDraw](codraw.nerdev.in) · **Source** [github.com/nerdev-co/codraw](https://github.com/nerdev-co/codraw/tree/main)
</Callout>
...
- extracting the canvas engine into focused managers (pointer, mouse, touch, keyboard, navigation, tool, text, shape, style, history, export, WebSocket sync, cursor, auto-save, laser, library, mermaid, plugin)
...
### Phase 5 — Manager Extraction (Aug 2026)

Extracted the canvas engine from a monolithic `Game.ts` (~2056 lines) into focused manager modules: `PointerInteractionManager`, `MouseManager`, `TouchManager`, `KeyboardManager`, `NavigationManager`, `ToolManager`, `TextManager`, `ShapeManager`, `ShapeArrangement`, `ShapeStyle`, `ArrowManager`, `ImageManager`, `ClipboardManager`, `RenderManager`, `AutoSaveManager`, `WebSocketSyncManager`, `CursorManager`, `HistoryManager`, `ExportManager`, `StyleManager`, `LaserManager`, `LibraryManager`, `MermaidManager`, and `PluginManager`. This reduced `Game.ts` to a thin public-API façade at ~1404 lines, with 23 files under `draw/managers/`.

### Phase 6 — Performance & Collaboration Hardening (Aug 2026)

Added spatial grid index for fast hit testing, cursor coalescing to reduce WebSocket message volume, trash persistence, image asset extraction for snapshot size control, frame highlight caching, scoped cache invalidation, and ChromeSlots layout coordinator.
...
| `ImageAsset` | Extracted image data URLs (UUID-keyed, keeps snapshots under 512KB) |
...
The Prisma `Session` model stores token hashes with expiration timestamps. On signin, the backend creates a session record and sets an httpOnly cookie with a 7-day session token. The response body only returns the `userId` — no JWT is sent to the client.

The WebSocket auth flow works around a constraint: WebSocket upgrade handshakes don't support custom Authorization headers in all browsers. Instead, the frontend calls `/auth/ws-token` (with the session cookie) to get a fresh 5-minute WS token in the response body, then passes it in the `Sec-WebSocket-Protocol` header. The WS backend verifies the session exists in Postgres before accepting the connection. Token-less guests can also connect without any auth, using a stable client-generated guest ID.
...
### 6. Manager extraction from monolith to focused modules

The canvas engine started as a single `Game.ts` file (~2056 lines) that owned every concern: pointer dispatch, mouse/touch/keyboard events, WebSocket sync, rendering, undo/redo, tool state, navigation, text editing, shape lifecycle, and export.

**Solution: decomposed into 23 focused manager modules under `draw/managers/`.**

Each manager owns a single concern and communicates through injected `GameContext` and API interfaces. Managers never cross-import each other. Pointer interaction, mouse, touch, keyboard, navigation, tool, text, shape arrangement, shape style, arrow, image, clipboard, render, auto-save, WebSocket sync, cursor, history, export, style, laser, library, mermaid, and plugin managers each handle their own lifecycle. `Game.ts` is now a thin public-API façade delegating to managers.

This makes the engine testable in isolation, reduces merge conflicts during parallel development, and makes each interaction path discoverable without reading a 2000-line file.
...
- **Spatial grid index** for fast hit testing on large canvases
- **ImageAsset extraction** to keep snapshot rows under the 512KB DB row cap
...
The deploy is idempotent: `pm2 start deploy/pm2/ecosystem.config.js --update-env` replaces the existing process without manual intervention.

### Incident Response

Three production incidents have been documented in the repository:
...
**Incident 3: Intermittent site failures — unbound SSR fetch + bad Nginx upgrade headers (2026-08-12)**

Shortly after the first production deploy, `codraw.nerdev.in` was intermittently unreachable: page loads would hang and fail, then work again moments later. The backend was healthy (`curl localhost:3001/` answered in 0.002s), OOM-kill was ruled out via `dmesg`, and PM2 logs showed clean boots — no crash loop. The real root cause was the home page's async server component: its SSR `fetch` to `/auth/me` (a database-backed route) had **no timeout**, so a slow/cold first query over the Neon serverless DB connection blocked the entire render until an internal timeout killed it, producing repeated `failed to pipe response` / `The operation timed out` (undici code 23) errors. A secondary problem: Nginx was sending `Connection: upgrade` on *every* request to the frontend, even plain GETs with no WebSocket upgrade.

**Fixed** by bounding the SSR fetch with `AbortSignal.timeout(5000)` so a slow backend degrades to guest rendering instead of hanging, and removing the unconditional `Upgrade`/`Connection: upgrade` headers from the `/` location (WebSocket proxying at `/ws` is unaffected). Root-caused via process-of-elimination: keep-alive fetch loop (no wedge), OOM check (empty), PM2 logs (clean), and `NEXT_PUBLIC_HTTP_BACKEND` being unset (ruled out a self-referential public-URL loop).

**Incident 4: Canvas Blur on HiDPI/Retina Displays (2026-08-14)**
...
**Root cause:** The canvas backing store was sized in CSS pixels rather than physical pixels. The engine's internal coordinate math was also sized in CSS pixels, but it read `canvas.width/height` directly as its logical dimensions — creating a coupling that prevented a one-line fix.

**Fixed** by separating the logical coordinate space (CSS pixels) from the physical backing store (CSS × DPR). New fields `cssWidth`, `cssHeight`, and `dpr` were added to `Game.ts`. All 24 logical `canvas.width/height` usages were replaced with `cssWidth/cssHeight`. The two sites that legitimately need the physical backing store dimensions (`buildCache` sizing, `clearCanvas` cache comparison) were left as `this.canvas.width/height`.

**Incident 5: TypeScript `boundTextId` Union Gap (2026-08-14)**
...
**Root cause:** `boundTextId` was added to the six container shape interfaces but not to the other shape interfaces in the `Shape` union type. TypeScript's discriminated union requires a property to exist on every member of the union before it can be accessed through a base-type reference.

**Fixed** by adding `boundTextId?: string` to all 12 shape interfaces in `packages/shapes/`.

**Incident 6: Cache Canvas Clear Rect Dimension Mismatch (2026-08-14)**
...
**Root cause:** `buildCache()` called `clearRect(0, 0, this.canvas.width, this.canvas.height)` on a DPR-transformed context. The code was mixing physical (`canvas.width`) and logical (`cssWidth`) values.

**Fixed** by replacing with logical dimensions and adding an explicit `ctx.setTransform(1,0,0,1,0,0)` before `drawImage` in `clearCanvas()` to ensure the cache blit is a raw 1:1 pixel copy.
...
| Commits | 474 |
| Lines of code | ~25,380 (excluding node_modules) |
| Drawing tools | 16 (Select, Hand, Rectangle, Diamond, Ellipse, Line, Arrow, Pencil, Eraser, Text, Image, Pen, Ellipsis arc, Sticky note, Frame, Eyedropper, Laser) |
| Export formats | 3 (PNG, SVG, JSON) |
| WebSocket message types | 8 (shape-diff, cursor, chat, join_room, leave_room, re_auth, presence, shape-diff-ack) |
| Session duration | 7 day session cookie, 5 min WS token |
...
- a fully functional canvas engine with 16 tools (Select, Hand, Rectangle, Diamond, Ellipse, Line, Arrow, Pencil, Eraser, Text, Image, Pen, Ellipsis arc, Sticky note, Frame, Eyedropper, Laser)
- real-time multi-user collaboration over WebSocket with guest access
- session-based authentication with httpOnly cookies and token revocation
- optimistic concurrency for conflict-free concurrent editing
- export to PNG, SVG, and JSON
...
- spatial grid index for fast hit testing on large canvases
- IndexedDB-backed LRU image cache with ImageAsset extraction for snapshot size control
...
- Extracted the canvas engine from a monolithic `Game.ts` (~2056 lines) into 23 focused manager modules under `draw/managers/`, reducing `Game.ts` to a thin public-API façade (~1404 lines).
...
- large-scale codebase refactoring (monolith to manager-based architecture without breaking existing behavior)
